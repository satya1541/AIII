...
export default function Connections() {
  ...
  const disconnectMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      toast({
        title: "Error", 
        description: "Failed to disconnect from MQTT broker",
        variant: "destructive",
      });
    },
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">MQTT Connections</h1>
          <p className="text-gray-400 mt-1">Manage your MQTT broker connections</p>
        </div>
        <Button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading ? (
          ...
          connections.map((connection: any) => (
            <Card key={connection.id} className="card-glass border-0 hover:bg-white hover:bg-opacity-5 transition-all duration-200">
