...
export default function RealtimeAnalytics({ connectionId, selectedTopic }: RealtimeAnalyticsProps) {
  ...
  const [liveStats, setLiveStats] = useState<LiveStats>(() => {
    ...
    try {
      ...
      console.log('Could not restore analytics stats');
    }
    return {
      messagesPerMinute: 0,
      averageValue: 0,
  ...
  const { data: connections = [] } = useQuery({
    ...
    refetchInterval: 2000, // Check connection status every 2 seconds
  }) as { data: any[] };
  ...
  const { data: messages = [], refetch: refetchMessages } = useQuery({
    ...
    refetchInterval: isRealTimeActive ? 5000 : false,
  ...
  const clearAnalyticsData = () => {
    console.log('Manually clearing analytics data');
    setRealtimeData([]);
    setMessageBuffer([]);
    setLiveStats({
      messagesPerMinute: 0,
        ...
        wsRef.current.onmessage = (event) => {
          try {
            ...
            if (data.type === 'mqtt_message') {
              ...
              console.log('Processing MQTT message for analytics:', message.topic, message.payload);
              
              // If no connection is selected, accept all messages
              // If connection is selected, filter by it
              ...
              if (extractedValues.length > 0) {
                ...
                setRealtimeData(prev => {
                  const updated = [...prev, ...newDataPoints].slice(-20); // Keep last 20 points
                  return updated;
                });
              }
  ...
  useEffect(() => {
    if (messages.length > 0) {
      ...
      if (!dataInitialized) {
        ...
        console.log('Initialized with', processedMessages.length, 'messages from database');
      }
  ...
  const updateLiveStats = (message: MessageData) => {
    ...
    setLiveStats((prev: LiveStats) => {
      ...
      try {
        ...
        console.log('Could not persist analytics stats');
      }
  ...
  const getLineChartData = () => {
    const timestamps = Array.from(new Set(realtimeData.map(d => d.timestamp))).slice(-10);
    const uniqueLabels = Array.from(new Set(realtimeData.map(d => d.label))).slice(0, 3);
    ...
    return {
      ...
      datasets: uniqueLabels.map((label, index) => ({
        ...
        data: timestamps.map(timestamp => {
          const point = realtimeData.find(d => d.timestamp === timestamp && d.label === label);
  ...
  const getBarChartData = () => {
    const labels = Array.from(new Set(realtimeData.map(d => d.label))).slice(0, 6);
  ...
  const getPieChartData = () => {
    ...
    return {
      labels: entries.map(([topic]) => topic.split('/').pop() || topic),
      datasets: [{
