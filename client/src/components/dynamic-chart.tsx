...
export default function DynamicChart({ topic }: DynamicChartProps) {
  ...
  useEffect(() => {
    if (lastMessage && lastMessage.type === 'mqtt_message' && topic) {
      ...
      console.log('WebSocket message received for dynamic chart:', msgData);
      ...
      if (msgData.topic === topic) {
        try {
          ...
          console.log('Extracted keys from payload:', extractedKeys);
          ...
          if (selectedKey && selectedKey in extractedKeys) {
            ...
            console.log('Adding new chart data point:', newPoint);
