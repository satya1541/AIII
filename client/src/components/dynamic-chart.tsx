...
export default function DynamicChart({ topic }: DynamicChartProps) {
  ...
  const { data: topicKeys, refetch: refetchKeys } = useQuery({
  ...
  const { data: keyValues, refetch: refetchValues } = useQuery({
  ...
  useWebSocket('/ws', (message) => {
    if (message.type === 'mqtt_message' && topic && selectedKey) {
      const msgData = message.data;
      if (msgData.topic === topic && msgData.extractedKeys) {
        try {
          const keys = msgData.extractedKeys;
          if (keys[selectedKey] !== undefined) {
            // Add new data point
            const newPoint = {
              value: keys[selectedKey],
  ...
  useEffect(() => {
    if (chartInstance.current && chartData.length > 0) {
      ...
      chart.data.datasets[0].data.push(
        typeof newData.value === 'number' ? newData.value : parseFloat(newData.value) || 0
      );
          ...
          <div>
            ...
            <Select value={selectedKey} onValueChange={setSelectedKey}>
              <SelectTrigger className="glass-morphism-dark border-0">
                <SelectValue placeholder="Choose a key to visualize" />
              </SelectTrigger>
              <SelectContent>
                {topicKeys && topicKeys.length > 0 ? (
                  topicKeys
                    ...
                    .map((key) => (
                      <SelectItem key={key.id} value={key.keyName}>
                        <div className="flex items-center space-x-2">
